...
public final class XtSafeBuffer implements AutoCloseable {
    ...
    static final Map<XtSample, Class<?>> _types = new HashMap<XtSample, Class<?>>(){
        {
            put(XtSample.UINT8, byte.class);
            put(XtSample.INT16, short.class);
            put(XtSample.INT24, byte.class);
            put(XtSample.INT32, int.class);
            put(XtSample.FLOAT32, float.class);
        }
    };

    public static XtSafeBuffer register(XtStream stream, boolean interleaved) {
        XtSafeBuffer result = new XtSafeBuffer(stream, interleaved);
